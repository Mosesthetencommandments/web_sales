  ...
  <div class="app-container calendar-list-container">
      ...
      <div class="filter-item">
       ...
       <div class="filter-item">
        ...
        <div class="filter-float">
            <el-button class="filter-item" type="primary" v-waves icon="el-icon-search" size="small" @click="handleFilter">搜索</el-button>
            <el-button class="filter-item" type="primary" v-waves icon="el-icon-edit" size="small" @click="handleCreate">添加</el-button>
            <el-button class="filter-item" type="primary" v-waves icon="el-icon-download" size="small" @click="downExcel">导出</el-button>
        </div>
      </div>
    </transition>
    <div :class="tableLoading">
      ...
      <el-tabs v-model="tabSelection" type="card" @tab-click="handleClick">
        <el-tab-pane :label="'需人工处理订单'" name="need_manual_handle" style="float:right;"></el-tab-pane>
      ...
      <sticky :sticky-top="74">
        <el-row>
          <el-col :span="7"><div class="grid-content bg-purple">商品</div></el-col>
          <el-col :span="2"><div class="grid-content bg-purple-light">买家</div></el-col>
          ...
          <el-col :span="3"><div class="grid-content bg-purple-light">加粉时间</div></el-col>
      ...
      <div v-for="orders in list" :key="orders.id">
        <div style="border:1px solid #ebeef5;width:100%;line-height:20px;padding:15px 20px;background-color:#f7f8fa;margin-bottom:-1px;color:#323233;">
          <div style="float:left;display:table-cell;line-height:30px;">
            <span>订单号： {{orders.orders_unique_id.slice(-8)}}</span>
            <el-tag size="mini" type="primary" v-if="orders.product_weixin_fans && orders.product_weixin_fans.join_time.slice(0,10) == orders.created_at.slice(0,10)">当日</el-tag>
          ...
          <div style="float:right;">
            ...
            <el-button v-if="orders.status===1" :loading="btnLoading" size="mini" type="success" @click="handleOpenOrdersDetail(orders)">确认订单</el-button>
            <el-button v-if="orders.status===1" size="mini" type="danger" @click="handleDestoryOrders(orders)">删除</el-button>
            <el-button v-if="orders.status===2" size="mini" type="info" @click="handleModifyStatus(orders, 1)">未确认</el-button>
          </div>
        ...
        <el-table :show-header="hideTableHeader" :key='tableKey2' :data="[orders]" element-loading-text="给我一点时间" border fit
          ...
          <el-table-column min-width="100" align="center" label="订单金额">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.status === 1" class="link-type" @click="handleOpenAnotherInner5(scope.row)" >修改价格</span>
            </template>
          </el-table-column>
          ...
          <el-table-column min-width="100" align="center" label="操作">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.status === 1" class="link-type" @click="handleOpenInner1(scope.row)" ><span>修改商品</span><br></span>
              <span v-if="scope.row.status === 1" class="link-type" @click="handleUpdate(scope.row)" ><span>修改收货信息</span><br></span>
              <span v-if="scope.row.status === 3 || scope.row.status === 4" class="link-type" @click="handleOpenAnotherInner5(scope.row)" ><span>申请改价</span><br></span>
              <span v-if="scope.row.status === 1 || scope.row.status === 6 || scope.row.status === 7 || scope.row.status === 8 || scope.row.status === 10" class="link-type" @click="handleOpenInner7(scope.row)" >
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm1" :model="temp" label-position="left" label-width="20%"
        ...
        <el-form-item label="收货人信息" v-if="!showReceiverForm">
          <el-input type="textarea" v-model="temp_receiver_info" :autosize="{ minRows: 6, maxRows: 20 }" placeholder="将客户姓名, 电话, 收货地址信息复制到这里, 点击智能识别, 系统自动识别并补全收货信息"></el-input>
        </el-form-item>
        ...
        <div v-if="showReceiverForm">
          ...
          <el-form-item label="仓库/发货时间" prop="warehouse_id">
            ...
            <p v-if="dialogStatus !== 'update'" style="margin:0;padding:0;text-align:right;">收货信息识别错误？ <span @click="handleUnidenfyReceiverInfo" class="link-type">点击上报</span></p>
          </el-form-item>
        </div>
      </el-form>
    ...
    <el-dialog title="退款详情" :visible.sync="innerDialogFormVisible7" width="50%">
      <el-form :rules="innerRules7" ref="innerDataForm7" :model="innerTemp7" label-position="left" label-width="120px"
            ...
            <el-table-column align="left" label="数量" min-width="150">
              <template slot-scope="scope">
                ...
                <div v-if="scope.row.editable">
                  <el-input-number v-model="scope.row.number" controls-position="right" :min="0" :max="100000" style="width:100%"></el-input-number>
                  <el-button class='inline-btn' v-if="scope.row.editable" type="success" @click="confirmChangeRefundItems(scope.row)" size="small" icon="el-icon-circle-check-outline">完成</el-button>
                  <el-button class='inline-btn' size="small" icon="el-icon-refresh" type="warning" @click="scope.row.editable = false">取消</el-button>
                </div>
                <span class="link-type" style="display:inline-block;text-align:center;" @click='scope.row.editable=!scope.row.editable' v-else>{{ scope.row.number }}</span>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-form-item label="收款人信息">
          <el-input type="textarea" v-model="innerTemp7.receiver_info" :autosize="{ minRows: 6, maxRows: 20 }"></el-input>
        </el-form-item>
        ...
        <el-form-item  v-if="innerTemp7.refund_img_url" label="转款截图">
            <img style="width: 100px; height: 100px" :src="innerTemp7.refund_img_url" @click="handleOpenImage(innerTemp7.refund_img_url)"/>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
    ...
    <el-dialog :title="innerTableTitle5" :visible.sync="innerTableVisible5" width="80%">
      ...
      <el-table :key='innerTableKey5' v-if="temp.orders_payment" :data="temp.orders_payment.orders_payment_items" v-loading="innerListLoading5"
                element-loading-text="给我一点时间" border fit highlight-current-row
                style="width: 100%">
        <el-table-column align="center" label="序号" min-width="65">
          <template slot-scope="scope">
        ...
        <el-table-column min-width="100px" align="center" label="收款方式">
          <template slot-scope="scope">
            <span class="link-type" v-if="scope.row.orders_pay_type.type_name">{{scope.row.orders_pay_type.type_name}}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="innerDialogFormVisible5" width="50%">
      <el-form :rules="innerRules5" ref="innerDataForm5" :model="innerTemp5" label-position="left" label-width="160px"
        ...
        <el-form-item label="付款凭证" prop="address" v-if="innerTemp5.orders_pay_type_id && innerTemp5.orders_pay_type_id !== -1 && innerTemp5.orders_pay_type_id !== 1 && innerTemp5.orders_pay_type_id !==2">
  ...
  export default {
    ...
    methods: {
      ...
      handleOpenPayProof() {
        ...
        getOrdersPayProofList({ paid_money: this.payProofQuery.paid_money, is_used: 0, pay_type_id: this.innerTemp5.orders_pay_type_id, orders_id: this.temp.id }).then(response => {
