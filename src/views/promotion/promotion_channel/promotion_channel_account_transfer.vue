<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :stripe="true" :key='innerTableKey2' :data="innerList2" v-loading="innerListLoading2" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column min-width="150px" align="center" label="转出账户">
        <template slot-scope="scope">
          <span style="font-weight: 600">{{scope.row.origin_promotion_channel_account.account_name}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80" align="center" label="转出返点">
        <template slot-scope="scope">
          <span>{{scope.row.origin_ratio}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80px" align="center" label="转出虚拟币">
        <template slot-scope="scope">
          <span>￥{{scope.row.origin_virtual_money}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80px" align="center" label="转出人民币">
        <template slot-scope="scope">
          <span>￥{{(scope.row.origin_virtual_money / scope.row.origin_ratio).toFixed(2)}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="150px" align="center" label="转入账户">
        <template slot-scope="scope">
          <span style="font-weight: 600">{{scope.row.target_promotion_channel_account.account_name}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80" align="center" label="转入返点">
        <template slot-scope="scope">
          <span>{{scope.row.target_ratio}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80px" align="center" label="转入虚拟币">
        <template slot-scope="scope">
          <span>￥{{scope.row.target_virtual_money}}</span>
        </template>
      </el-table-column>
      <el-table-column width="80px" align="center" label="转入人民币">
        <template slot-scope="scope">
          <span>￥{{(scope.row.target_virtual_money / scope.row.target_ratio).toFixed(2)}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="操作人">
        <template slot-scope="scope">
          <span>{{scope.row.promotion_user_account.nickname}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="130" align="center" label="时间">
        <template slot-scope="scope">
...
<script>
  import { getPromotionChannelAccountList, createPromotionChannelAccount, updatePromotionChannelAccount, deletePromotionChannelAccount, createPromotionChannel, updatePromotionChannel, getPromotionChannelList, createPromotionChannelAccountConsumption, createPromotionChannelAccountRecharge, deletePromotionChannelAccountRecharge, getPromotionChannelAccountRechargeList } from '@/api/promotion_channel'
  import { getProductList } from '@/api/product'
  import { getPromotionChannelAccountTransferList } from '@/api/promotion_channel'
  import { checkPromotionChannelAccountTransfer } from '@/api/promotion_statistics'
  ...
  export default {
    ...
    methods: {
      ...
      getInnerList2() {
        ...
        getPromotionChannelAccountTransferList(this.innerListQuery2).then(response => {
      ...
      handleInnerDelete2(row, status) {
        this.$confirm('是否继续?', '提示', {
          ...
          const ttempData = {
            promotion_channel_account_transfer_id: row.id,
            check_status: status
          }
          checkPromotionChannelAccountTransfer(ttempData).then(response => {
            const index = this.innerList2.indexOf(row)
            this.innerList2[index].check_status = status
          })
        }).catch(() => {
          this.$message({
            type: 'info',
