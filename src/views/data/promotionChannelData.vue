    ...
    <el-table :span-method="objectSpanMethod" id="channelTable" :height="tableHeight" show-summary
      ...
      <el-table-column :key="Math.random()" fixed align="center" label="客服" min-width="130">
        <template slot-scope="scope">
          ...
          <br/>
          <span v-if="scope.row.support_user_account">{{scope.row.support_user_account.created_at.slice(0,10)}}入职</span>
        </template>
      </el-table-column>
    ...
    methods: {
      ...
      getList() {
        ...
        getSupportChannelStatisticsDataMonth(this.listQuery).then(res => {
          ...
          this.list.forEach(item => {
            if (item.support_user_account == null || !item.support_user_account || !item.support_user_account.created_at ) {
              console.log('1111111111111',item)
            }
          })
          if (this.list.support_user_account == null)
