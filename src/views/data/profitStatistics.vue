<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey'  id="cost" :stripe="true" show-summary :summary-method="getPromotionChannelSummaries" :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
              style="width: 100%;" >
      <el-table-column :key="Math.random()" align="center" label="客服" min-width="50">
        <template slot-scope="scope">
              ...
              <br>
              <span>
                ({{scope.row.support_member.user_account_group.group_name}})
              </span>
        </template>
      </el-table-column>
      <el-table-column :key="Math.random()" align="center" label="佣金" min-width="100">
        <template slot-scope="scope">
              <span style="font-size:16px;color:red;">
                ¥ {{Math.round(scope.row.profit * 100) / 100}}
              </span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  import { getSalesShopCustomerProfitStatistics } from '@/api/shop_statistics'
    ...
    computed: {
      getPromotionChannelSumData: function() {
        ...
        for (const v of this.list) {
          innerTempData.profit += parseFloat(v.profit)
        }
        ...
        // }
        sums[0] = '合计'
        sums[1] = '￥' + Math.round(innerTempData.profit) // 代收
        return sums
      }
    },
    methods: {
      ...
      getList() {
        ...
        getSalesShopCustomerProfitStatistics(this.listQuery).then(res => {
          this.list = []
