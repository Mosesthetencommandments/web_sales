<template>
  <div class="comp-full-calendar">
    ...
    <div class="full-calendar-body">
      <div class="weeks">
        <p class="week" v-for="dayIndex in 7">{{ (dayIndex - 1) | localeWeekDay(firstDay, locale) }}</p>
      </div>
            ...
            <div class="events-day" v-for="day in week" track-by="$index"
              ...
              <div class="event-box">
                <event-card :event="event" :date="day.date" :firstDay="firstDay" v-for="event in day.events" v-show="event.cellIndex <= eventLimit" @click="eventClick">
                  <!--<template scope="p">-->
                    <!--<slot name="fc-event-card" :event="p.event">-->
                    <!--</slot>-->
                  <!--</template>-->
                </event-card>
              ...
              <p v-if="day.events.length > eventLimit"
                      class="more-link" @click.stop="selectThisDay(day, $event)">
                more +{{day.events[day.events.length -1].cellIndex - eventLimit}}
              </p>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods : {
      ...
      selectThisDay (day, jsEvent) {
        let _day = JSON.parse(JSON.stringify(day))
        _day.events.splice(0,this.eventLimit)
        this.selectDay = _day;
        this.showMore = true;
